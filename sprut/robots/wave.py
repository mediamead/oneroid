...
np.set_printoptions(formatter={'all':lambda x: "%0.3f" % x})
...
if __name__ == "__main__":
    ...
    if True:
        ...
        vout = cv2.VideoWriter('wave.avi', fourcc, sps/10, (W*2, H*2))
    else:
        vout = None
    ...
    while not gui or (p.readUserDebugParameter(s_quit) == 0):
        ...
        if kb.kbhit():
            ...
            if key == 'q':
                ...
                wave_axis = ord(key) - ord('0')
                print("Selected %d axis" % wave_axis)
            elif wave_axis is not None:
                wave_changed = True
                if key == 'f':
                    wave_phase[wave_axis] -= (2*np.pi)/FSTEPS
                elif key == 'F':
                    wave_phase[wave_axis] += (2*np.pi)/FSTEPS
                elif key == 'a':
                    wave_ampl[wave_axis] -= (2*np.pi)/ASTEPS
                elif key == 'A':
                    wave_ampl[wave_axis] += (2*np.pi)/ASTEPS
                elif key == 'b':
                    wave_b[wave_axis] -= (2*np.pi)/BSTEPS
                elif key == 'B':
                    wave_b[wave_axis] += (2*np.pi)/BSTEPS
                else:
                    wave_changed = False
                ...
                if wave_changed:
                    print("wave_ampl : %s" % wave_ampl)
                    print("wave_phase: %s" % wave_phase)
                    print("wave_b    : %s" % wave_b)
